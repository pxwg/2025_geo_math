...
We would consider the perturbative theory of a scaler field theory, where $ \mathcal{E} = C^{\infty }(X)$, $ X = \mathbb{R}^{d}$ and the action is given by
\begin{equation*}
...
\end{equation*}
where $ \iota_{V_f} \omega = \mathrm{d} f$. The deformation quantization would lead to a non-communitative, where the communitve product in $ C^{\infty }(M)$ is replaced by the Moyal product:
\begin{equation*}
