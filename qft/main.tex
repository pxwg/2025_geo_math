...
Their is a alernative way to define the Noether current, which is more suitable for practical use. In brief, on can consider the 'gauged' symmetry which would promote $ \epslion$ to become a field $ \epsilon(x)$, and the variation of the action could be compute by integrating by parts, finally one can obtain
\begin{equation*}
