...
% 额外计数的可能性，因此需要考虑对称性
Using diagram to represent the process of Wick's contraction, one might meeting the problem of over-counting.
A proper way to avoid the over-counting is to consider the symmetry of the diagram i.e., the automorphism of a diagram and isomorphisms between different diagrams.
...
Consider the theory in this section, given a graph $ \Gamma$, we can associate a Feynman diagram to it, which is a collection of vertices and edges, where each vertex corresponds to a interaction term $ I(x)$ and each edge corresponds to a contraction between two fields, which would correspond to $ Q^{-1}$.
...
\end{equation*}
then the series expension will have all possible graphs, which could be computed by the Feynman rules
\begin{equation*}
